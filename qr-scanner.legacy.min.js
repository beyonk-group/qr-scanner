...
c._preferredCamera&&(c._preferredCamera=a,yield c._restartVideoStream())})}static scanImage(a,c={}){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let {scanRegion:d,qrEngine:f,canvas:g,disallowCanvasResizing:k=!1,alsoTryWithoutScanRegion:m=!1}=c,p=!!f;try{let q,n;[f,q]=yield Promise.all([f||b.createQrEngine(),b._loadImage(a)]);[g,n]=b._drawToCanvas(q,d,g,k);let r;if(f instanceof Worker){let h=f;p||b._postWorkerMessageSync(h,"inversionMode","both");r=yield new Promise((l,w)=>{let x,
v,t,z=-1;v=u=>{u.data.id===z&&(h.removeEventListener("message",v),h.removeEventListener("error",t),clearTimeout(x),null!==u.data.data?l({data:u.data.data,cornerPoints:b._convertPoints(u.data.cornerPoints,d)}):w(b.NO_QR_CODE_FOUND))};t=u=>{h.removeEventListener("message",v);h.removeEventListener("error",t);clearTimeout(x);w("Scanner error: "+(u?u.message||u:"Unknown Error"))};h.addEventListener("message",v);h.addEventListener("error",t);x=setTimeout(()=>t("timeout"),1E4);let y=n.getImageData(0,0,g.width,
g.height);z=b._postWorkerMessageSync(h,"decode",y,[y.data.buffer])})}else r=yield Promise.race([new Promise((h,l)=>window.setTimeout(()=>l("Scanner error: timeout"),1E4)),(()=>$jscomp.asyncExecutePromiseGeneratorFunction(function*(){try{var [h]=yield f.detect(g);if(!h)throw Error(b.NO_QR_CODE_FOUND);return{data:h.rawValue,cornerPoints:b._convertPoints(h.cornerPoints,d)}}catch(l){h=l.message||l;if(/not implemented|service unavailable/.test(h))return b._disableBarcodeDetector=!0,b.scanImage(a,{scanRegion:d,
canvas:g,disallowCanvasResizing:k,alsoTryWithoutScanRegion:m});throw Error(`Scanner error: ${h}`);}}))()]);return r}catch(q){if(!d||!m)throw q;return yield b.scanImage(a,{qrEngine:f,canvas:g,disallowCanvasResizing:k})}finally{p||b._postWorkerMessage(f,"close")}})}setGrayscaleWeights(a,c,d,f=!0){b._postWorkerMessage(this._qrEnginePromise,"grayscaleWeights",{red:a,green:c,blue:d,useIntegerApproximation:f})}setInversionMode(a){b._postWorkerMessage(this._qrEnginePromise,"inversionMode",a)}static createQrEngine(){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let a=
()=>Promise.resolve().then(function(){return e}).then(d=>d.createWorker());if(!(!b._disableBarcodeDetector&&"BarcodeDetector"in window&&BarcodeDetector.getSupportedFormats&&(yield BarcodeDetector.getSupportedFormats()).includes("qr_code")))return a();let c=navigator.userAgentData;return c&&c.brands.some(({brand:d,version:f})=>/Chromium/i.test(d)&&113>parseInt(f))&&/mac ?OS/i.test(c.platform)&&(yield c.getHighEntropyValues(["architecture","platformVersion"]).then(({architecture:d,platformVersion:f})=>
/arm/i.test(d||"arm")&&13<=parseInt(f||"13")).catch(()=>!0))?a():new BarcodeDetector({formats:["qr_code"]})})}_onPlay(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay();this.$overlay&&(this.$overlay.style.display="");this._scanFrame()}_onLoadedMetaData(){this._scanRegion=this._calculateScanRegion(this.$video);this._updateOverlay()}_onVisibilityChange(){document.hidden?this.pause():this._active&&this.start()}_calculateScanRegion(a){let c=Math.round(2/3*Math.min(a.videoWidth,
a.videoHeight));return{x:Math.round((a.videoWidth-c)/2),y:Math.round((a.videoHeight-c)/2),width:c,height:c,downScaledWidth:this._legacyCanvasSize,downScaledHeight:this._legacyCanvasSize}}_updateOverlay(){requestAnimationFrame(()=>{if(this.$overlay){var a=this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,k=a.offsetLeft,m=a.offsetTop,p=window.getComputedStyle(a),q=p.objectFit,n=c/d,r=f/g;switch(q){case "none":var h=c;var l=d;break;case "fill":h=f;l=g;break;default:("cover"===
q?n>r:n<r)?(l=g,h=l*n):(h=f,l=h/n),"scale-down"===q&&(h=Math.min(h,c),l=Math.min(l,d))}var [w,x]=p.objectPosition.split(" ").map((t,z)=>{const y=parseFloat(t);return t.endsWith("%")?(z?g-l:f-h)*y/100:y});p=this._scanRegion.width||c;r=this._scanRegion.height||d;q=this._scanRegion.x||0;var v=this._scanRegion.y||0;n=this.$overlay.style;n.width=`${p/c*h}px`;n.height=`${r/d*l}px`;n.top=`${m+x+v/d*l}px`;d=/scaleX\(-1\)/.test(a.style.transform);n.left=`${k+(d?f-w-h:w)+(d?c-q-p:q)/c*h}px`;n.transform=a.style.transform}})}static _convertPoints(a,
c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let k of a)k.x=k.x*g+d,k.y=k.y*c+f;return a}_scanFrame(){!this._active||this.$video.paused||this.$video.ended||("requestVideoFrameCallback"in this.$video?this.$video.requestVideoFrameCallback.bind(this.$video):requestAnimationFrame)(()=>{const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(!(1>=a.$video.readyState)){var c=
Date.now()-a._lastScanTimestamp,d=1E3/a._maxScansPerSecond;c<d&&(yield new Promise(g=>setTimeout(g,d-c)));a._lastScanTimestamp=Date.now();try{var f=yield b.scanImage(a.$video,{scanRegion:a._scanRegion,qrEngine:a._qrEnginePromise,canvas:a.$canvas})}catch(g){if(!a._active)return;a._onDecodeError(g)}!b._disableBarcodeDetector||(yield a._qrEnginePromise)instanceof Worker||(a._qrEnginePromise=b.createQrEngine());f?(a._onDecode?a._onDecode(f):a._legacyOnDecode&&a._legacyOnDecode(f.data),a.$codeOutlineHighlight&&
(clearTimeout(a._codeOutlineHighlightRemovalTimeout),a._codeOutlineHighlightRemovalTimeout=void 0,a.$codeOutlineHighlight.setAttribute("viewBox",`${a._scanRegion.x||0} `+`${a._scanRegion.y||0} `+`${a._scanRegion.width||a.$video.videoWidth} `+`${a._scanRegion.height||a.$video.videoHeight}`),a.$codeOutlineHighlight.firstElementChild.setAttribute("points",f.cornerPoints.map(({x:g,y:k})=>`${g},${k}`).join(" ")),a.$codeOutlineHighlight.style.display="")):a.$codeOutlineHighlight&&!a._codeOutlineHighlightRemovalTimeout&&
